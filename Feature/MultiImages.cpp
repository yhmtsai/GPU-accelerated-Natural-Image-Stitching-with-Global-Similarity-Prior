...
vector<pair<int, int> > MultiImages::getInitialFeaturePairs(const pair<int, int> & _match_pair) const {
    ...
    float *gpu_data1=NULL, *gpu_data2=NULL, *AB=NULL, *gpu_norm1=NULL, *gpu_norm2=NULL, *gpu_ones=NULL;
    ...
    for(int p = 0; p < pair_count; ++p) {
        ...
        vector<FeatureDescriptor> & feature_descriptors_1 = images_data[pair_match[ p]].getFeatureDescriptors();
        vector<FeatureDescriptor> & feature_descriptors_2 = images_data[pair_match[!p]].getFeatureDescriptors();
        int size_1=0, size_2=0, index_1=0, index_2=0;
        for (int i=0; i<feature_size[p]; i++){
            size_1+=feature_descriptors_1[i].data.size();
            for (int j=0; j<feature_descriptors_1[i].data.size(); j++){
                cudaHostRegister(feature_descriptors_1[i].data[j].ptr<vl_sift_pix>(0), SIFT_DESCRIPTOR_DIM* sizeof(float), cudaHostRegisterDefault);
            }
            
        }
        for (int i=0; i<feature_size[!p]; i++){
            size_2+=feature_descriptors_2[i].data.size();
            for (int j=0; j<feature_descriptors_2[i].data.size(); j++){
                cudaHostRegister(feature_descriptors_2[i].data[j].ptr<vl_sift_pix>(0), SIFT_DESCRIPTOR_DIM* sizeof(float), cudaHostRegisterDefault);
            }
        }
        int mn=max(size_1, size_2);
        cudaMalloc((void**) &gpu_data1, SIFT_DESCRIPTOR_DIM*size_1*sizeof(float));
        cudaMalloc((void**) &gpu_data2, SIFT_DESCRIPTOR_DIM*size_2*sizeof(float));
        cudaMalloc((void**) &gpu_norm1, size_1*sizeof(float));
        cudaMalloc((void**) &gpu_norm2, size_2*sizeof(float));
        cudaMalloc((void**) &AB, size_1*size_2*sizeof(float));
        cudaMalloc((void**) &gpu_ones, mn*sizeof(float));
        float *norm1=new float[size_1], *norm2=new float[size_2], *ones=new float[mn];
        for (int i=0; i<feature_size[p]; i++){
            for (int j=0; j<feature_descriptors_1[i].data.size(); j++){
                cudaMemcpyAsync(gpu_data1+index_1*SIFT_DESCRIPTOR_DIM, feature_descriptors_1[i].data[j].ptr<vl_sift_pix>(0),
                                SIFT_DESCRIPTOR_DIM*sizeof(float), cudaMemcpyHostToDevice, stream[index_1%num_streams]);
                cublasSnrm2(cublas_handle[index_1%num_streams], SIFT_DESCRIPTOR_DIM, 
                            gpu_data1+index_1*SIFT_DESCRIPTOR_DIM, 1, norm1+index_1);
                norm1[index_1]*=norm1[index_1];
                // cout<<norm1[index_1]<<" ";
                // float tn=0;
                // for (int k=0; k<SIFT_DESCRIPTOR_DIM; k++){
                //     tn+=feature_descriptors_1[i].data[j].at<vl_sift_pix>(k)*feature_descriptors_1[i].data[j].at<vl_sift_pix>(k);
                // }
                // cout<<tn<<" ";
                index_1++;
            }
        ...
        for (int i=0; i<feature_size[!p]; i++){
            for (int j=0; j<feature_descriptors_2[i].data.size(); j++){
                cudaMemcpyAsync(gpu_data2+index_2*SIFT_DESCRIPTOR_DIM, feature_descriptors_2[i].data[j].ptr<vl_sift_pix>(0),
                                SIFT_DESCRIPTOR_DIM*sizeof(float), cudaMemcpyHostToDevice, stream[index_2%num_streams]);
                cublasSnrm2(cublas_handle[index_2%num_streams], SIFT_DESCRIPTOR_DIM, 
                            gpu_data2+index_2*SIFT_DESCRIPTOR_DIM, 1, norm2+index_2);
                norm2[index_2]*=norm2[index_2];
        ...
        cudaMemcpyAsync(gpu_norm1, norm1, size_1*sizeof(float), cudaMemcpyHostToDevice, stream[1]);
        cudaMemcpyAsync(gpu_norm2, norm2, size_2*sizeof(float), cudaMemcpyHostToDevice, stream[1]);
        cudaMemcpyAsync(gpu_ones, ones, mn*sizeof(float), cudaMemcpyHostToDevice, stream[1]);
        cublasSgemm(cublas_handle[0], CUBLAS_OP_T, CUBLAS_OP_N, size_1, size_2, SIFT_DESCRIPTOR_DIM, 
                    &alpha, gpu_data1, SIFT_DESCRIPTOR_DIM, gpu_data2, SIFT_DESCRIPTOR_DIM, 
                    &zero, AB, size_1);
        cudaDeviceSynchronize();
        alpha=1;
        cublasSger( cublas_handle[0], size_1, size_2, &alpha, gpu_norm1, 1, gpu_ones, 1, AB, size_1);
        cudaDeviceSynchronize();
        cublasSger( cublas_handle[0], size_1, size_2, &alpha, gpu_ones, 1, gpu_norm2, 1, AB, size_1);
        cudaDeviceSynchronize();
        float *dist_mat=new float [size_1*size_2];
        cudaMemcpy( dist_mat, AB, size_1*size_2*sizeof(float), cudaMemcpyDeviceToHost);
        index_1=0;
        ...
        for(int f1 = 0; f1 < feature_size[p]; ++f1) {
            ...
            for(int f2 = 0; f2 < feature_size[!p]; ++f2) {
                ...
                float temp_dist=10;
                // cout<<"real:"<<dist<<" | ";
                for (int i=index_1; i<index_1+feature_descriptors_1[f1].data.size(); i++){
                    for (int j=index_2; j<index_2+feature_descriptors_2[f2].data.size(); j++){
                        // cout<<"("<<i<<","<<j<<","<<dist_mat[j*size_1+i]<<") ";
                        temp_dist=min(temp_dist, dist_mat[j*size_1+i]);
                    }
                }
                // cout<<temp_dist<<"\n";
                const double dist=temp_dist;
                // cin.get();
                ...
                index_2+=feature_descriptors_2[f2].data.size();
            }
            index_1+=feature_descriptors_1[f1].data.size();
